	...
	chrome.runtime.onMessage.addListener(
		function(request, sender, sendResponse) {
			if (request.message === "browser_action_commit" ) {
				console.log('Received message: browser_action_commit, sink_no: ' + request.sink_no);
				...
				console.log('Received message: report_sink_no');
				//log('Reply with: sink_no: ' + sink_no+' sink_no: '+output_sink_no);
...
function check_node(node) {
	...
	if ((name == 'AUDIO') || (name == 'VIDEO')) {
		console.log('node added/changed: ' + name);
		return true;
	}
	return false;
}
...
function request_default_no() {
	...
	chrome.runtime.sendMessage({'method': 'AP_get_default_no'},
		function(response) {
			if (response) {
				console.log('Received default_no: ' + response.default_no);
...
function inspect_devices(deviceInfos) {
	...
	for (var i = 0; i != deviceInfos.length; i++) {
		...
		//console.log('  Devices[' + i + ']: ' + deviceInfo.kind + ': ' + deviceInfo.deviceId);
		if ((deviceInfo.kind == 'audiooutput') && (i == sink_no)) {
			console.log('Selecting Devices[' + i + ']: ' + deviceInfo.deviceId);
...
function update_all_sinks() {
	...
	for (var j = 0; j < allMedia.length; j++) {
		...
		console.log('  Queuing SetSinkId: ' + j + ': ' + name + ': ' +  src + ': ' + sink_id);
		promises.push(allMedia[j].setSinkId(sink_id));
	}
	if (promises.length > 0) {
		console.log('Tyring to update all (' + promises.length + ') sinks (GUM_state == ' + GUM_state + '): ' + sink_id);
		Promise.all(promises)
			.then(function(results){log('All set.'); })
			.catch(function(error){
				if (GUM_state == undefined) {
					GUM_state = 0;
					console.log('SetSinkId failed: ' + error + '. Retrying with GUM ...');
					with_or_without_GUM();
				} else {
					console.log('SetSinkId failed: ' + error + '.  Giving up.');
				}
			});
	} else {
