...
// -- Help the background page in retrieving devices and update the default device selection page 
 function init() { 	
	navigator.webkitGetUserMedia(
		...
		function(stream) {
			console.log('getUserMedia: success (as it should be, when called from options full page)');
			navigator.mediaDevices.enumerateDevices()
				.then(getDevices)
				.catch(errorCallback);
		},
		function(stream) {
			console.log('getUserMedia: error (as is expected, when run from options_ui)');
			navigator.mediaDevices.enumerateDevices()
				.then(getDevices)
				.catch(errorCallback);
		}
...
function update_default_no(e) {
	console.log("update_default_no: " +  e.target.value);
	var default_no =  bg.document.getElementById("default_no");
	default_no.value = e.target.value;
    chrome.storage.local.set({"IP_default_no" : e.target.value});
    
}


const audioOutputSelect = document.getElementById('device_options');
const audioOutputbg= bg.document.querySelector('device_cache');
...
function getDevices(deviceInfos) {
  ...
  console.log('update_device_options: ' + deviceInfos.length + ' device(s) total (audio/video input/output)');
  var div = document.getElementById("device_options");
  var select = bg.document.getElementById("device_cache");
  var default_no = bg.document.getElementById("default_no");
  while (div.firstChild) { div.removeChild(div.firstChild); }
